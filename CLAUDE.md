# CLAUDE.md - ריפו `agency` (הוראות חובה)

הריפו הזה (github.com/ilana-ai/agency) מתפרסם ב-GitHub Pages על הדומיין **`https://ilana-ai.agency/`**.
כל תיקיית עמוד מוגשת **משורש הדומיין**: `https://ilana-ai.agency/<שם-התיקייה>/`. HTTPS נאכף,
והכתובות הישנות ב-`ilana-ai.github.io/pages/` מפנות לכאן אוטומטית.

⚠️ **קובץ `CNAME` בשורש הריפו הוא מה שמחזיק את הדומיין. אסור למחוק או לשנות אותו.**
...
**כללי ברזל:**
- כל תיקיית עמוד היא **עצמאית** - עותק משלה של קבצי ה-favicon, כדי שכל עמוד יעמוד בפני עצמו.
...
מקורות הלוגו יושבים **מחוץ לריפו הזה**, בריפו הפרטי `ilana-agents`, תחת `pages/logo/מוקטן במשקל/`:
